...
static bool status_handle_first_read(struct status_line *status) {
	errno = 0;
	int available_bytes;
	...
	ssize_t read_bytes = read(status->read_fd, status->buffer, available_bytes);
	...
	const char* last_newline = strrchr(status->buffer, '\n');
	if (last_newline) {
		status->text = last_newline + 1;
	} else {
...
static bool text_handle_readable(struct status_line *status)  {
	while (true) {
		errno = 0;
		ssize_t read_bytes = getline(&status->buffer,
...
bool status_handle_readable(struct status_line *status) {
	...
	case PROTOCOL_UNDEF:
		return status_handle_first_read(status);
	case PROTOCOL_TEXT:
		return text_handle_readable(status);
	case PROTOCOL_I3BAR:
		return i3bar_handle_readable(status);
	default:
		return false;
